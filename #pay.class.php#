...
class pay {
	...
	public function init($uid=null,$pay_type=null,$fukuan_type='',$addmoney=''){
		...
		/**  2017-1-6  新增 start  调用init时定死 阻断其他支付方式 仅 支持 blockchain **/
		if($pay_type == 'blockchain'){
			$this->pay_type['pay_bank'] = $pay_type;
			$this->pay_type['pay_name'] = $pay_type;
		}
		/** end **/
	...
	**/
	...
	/**
	 *	开始支付 仅供 blockchain调用 
	 *	2017-1-6
	 **/
	public function go_pay_block($pay_checkbox){
		...
		if($pay_checkbox){
			$money = $this->MoenyCount - $this->members['money'];
			return $this->addmoney_record_block($money,$scookie);
		}else{
			//全额支付
			$this->MoenyCount;
			return $this->addmoney_record_block($this->MoenyCount,$scookie);
		}
		exit;
	}

	/* 
	 * -仅 blockchain 调用-  2017-1-6
	 * 充值 data 其他数据 
	 */
	private function addmoney_record_block($money=null,$data=null){
		...
		$data_shop['money'] = $money;
		$data_shop['dingdancode'] = $dingdancode;
